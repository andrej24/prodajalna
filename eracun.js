...
streznik.post('/izpisiRacunBaza', function(zahteva, odgovor) {
  ...
  form.parse(zahteva, function (napaka1, polja, datoteke) {
    ...
    try {
      ...
      pesmiIzRacuna(id,function(res){
        strankaIzRacuna(id,function(customer){
            odgovor.setHeader('content-type', 'text/xml');
            odgovor.render('eslog', {
            vizualiziraj: true,
            postavkeRacuna: res,
            postavkeNarocnika: customer
          })  
        });
      });
    } catch (err) {
...
var stranka = function(id,callback) {
...
streznik.get('/izpisiRacun/:oblika', function(zahteva, odgovor) {
  ...
  form.parse(zahteva, function (napaka1, polja, datoteke) {
    var napaka2 = false;
    try {
        stranka(zahteva.session.customer,function(res){
          console.log(res);
          pesmiIzKosarice(zahteva, function(pesmi) {
            if (!pesmi) {
              odgovor.sendStatus(500);
...
streznik.post('/stranka', function(zahteva, odgovor) {
  var form = new formidable.IncomingForm();
  form.parse(zahteva, function (napaka1, polja, datoteke) {
    zahteva.session.customer = polja.seznamStrank;
    odgovor.redirect('/')
  });
})
...
streznik.post('/odjava', function(zahteva, odgovor) {
    zahteva.session.customer = null;
    odgovor.redirect('/prijava') 
})

function signIn(zahteva,odgovor){
  var v = false;
  if(zahteva.session.customer!=null){
      v=true;
  }else{
      v=false;
  }
